...
module.exports = {
  ...
  execute: async function ({ client, message, args }) {
    ...
    let commandAlias = guildInfo.commandAlias
      ? Object.entries(guildInfo.commandAlias)
      : [];
    let commands = new Collection();
    const embed = (
      await CustomEmbed({ client: client, userID: message.author.id })
    ).setTitle("Custom command aliases");

    if (!args[0]) {
      if (commandAlias.length === 0) {
        embed.setDescription(
          "There are no custom aliases for this server set yet."
        );
      } else {
      ...
      switch (action) {
        case "set":
          if (args.length < 2) {
            embed.setDescription(
              `${message.author}, please check the usage of the command.`
            );
            break;
          }
          ...
          aliasAlreadyExists =
            ...
            (guildInfo.commandAlias ? guildInfo.commandAlias[args[0]] : false);
          ...
          if (aliasAlreadyExists) {
            ...
            embed.setDescription(
              `${
                message.author
              }, this alias is already in use for the command: \`${
                // @ts-ignore
          ...
          if (!command) {
            embed.setDescription(
              `${message.author}, the command \`${args[1]}\` doesn't exist.`
            );
            break;
          }
          ...
          if (command.canNotAddAlias) {
            embed.setDescription(
              `${message.author}, you can not add aliases to the command \`${args[1]}\`.`
            );
          }
          ...
          guildInfo = await client.DBGuild.findByIdAndUpdate(
            message.guild.id,
            { $set: { commandAlias: guildInfo.commandAlias } },
            { new: true, upsert: true, setDefaultsOnInsert: true }
          );

          embed.setDescription(
            `${message.author}, the command \`${command.name}\` has been given the new alias \`${args[0]}\``
          );
          break;
        ...
        case "remove":
          ...
          if (!aliasAlreadyExists) {
            embed.setDescription(
              `${message.author}, that alias doesn't exist yet.`
            );
            break;
          }
          ...
          embed.setDescription(
            `${message.author}, the alias \`${args[0]}\` has been removed.`
          );
          break;
