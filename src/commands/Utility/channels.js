  ...
  execute: async function ({ client, message, args }) {
    ...
    if (!args[1])
      return message.channel.send(
        channelEmbed.setDescription("**Please specify a channel to disable.**")
      );
    ...
    if (!channel)
      return message.channel.send(
        channelEmbed.setDescription(
          `**The channel ${args[1]} does not exist.**`
        )
      );
    if (channel.type !== "text")
      return message.channel.send(
        channelEmbed.setDescription(`**You can only disable text channels.**`)
      );
    ...
    switch (args[0]) {
      case "disable":
        if (disabledChannels.includes(channel.id))
          return message.channel.send(
            channelEmbed.setDescription(
              `**The channel ${channel} is already disabled.**`
            )
          );
        ...
        message.channel.send(
          channelEmbed.setDescription(
            `**The channel ${channel} has been disabled.**`
          )
        );
        break;
      ...
      case "enable":
        if (!disabledChannels.includes(channel.id))
          return message.channel.send(
            channelEmbed.setDescription(
              `**The channel ${channel} is already enabled.**`
            )
          );
        ...
        message.channel.send(
          channelEmbed.setDescription(
            `**The channel ${channel} has been enabled.**`
          )
        );
        break;
